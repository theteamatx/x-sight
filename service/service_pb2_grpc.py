# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""Client and server classes corresponding to protobuf-defined services."""
import grpc

from service import service_pb2 as service_dot_service__pb2


class SightServiceStub(object):
    """This API manages Sight logs, their creation and access to them.
    """

    def __init__(self, channel):
        """Constructor.

        Args:
            channel: A grpc.Channel.
        """
        self.Test = channel.unary_unary(
                '/sight.x.service.SightService/Test',
                request_serializer=service_dot_service__pb2.TestRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.TestResponse.FromString,
                )
        self.PrintInsertionTime = channel.unary_unary(
                '/sight.x.service.SightService/PrintInsertionTime',
                request_serializer=service_dot_service__pb2.TestRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.TestResponse.FromString,
                )
        self.Create = channel.unary_unary(
                '/sight.x.service.SightService/Create',
                request_serializer=service_dot_service__pb2.CreateRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.CreateResponse.FromString,
                )
        self.Launch = channel.unary_unary(
                '/sight.x.service.SightService/Launch',
                request_serializer=service_dot_service__pb2.LaunchRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.LaunchResponse.FromString,
                )
        self.GetWeights = channel.unary_unary(
                '/sight.x.service.SightService/GetWeights',
                request_serializer=service_dot_service__pb2.GetWeightsRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.GetWeightsResponse.FromString,
                )
        self.DecisionPoint = channel.unary_unary(
                '/sight.x.service.SightService/DecisionPoint',
                request_serializer=service_dot_service__pb2.DecisionPointRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.DecisionPointResponse.FromString,
                )
        self.CopyDataToReplayServer = channel.unary_unary(
                '/sight.x.service.SightService/CopyDataToReplayServer',
                request_serializer=service_dot_service__pb2.CopyDataToReplayServerRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.CopyDataToReplayServerResponse.FromString,
                )
        self.CurrentStatus = channel.unary_unary(
                '/sight.x.service.SightService/CurrentStatus',
                request_serializer=service_dot_service__pb2.CurrentStatusRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.CurrentStatusResponse.FromString,
                )
        self.ProposeAction = channel.unary_unary(
                '/sight.x.service.SightService/ProposeAction',
                request_serializer=service_dot_service__pb2.ProposeActionRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.ProposeActionResponse.FromString,
                )
        self.FinalizeEpisode = channel.unary_unary(
                '/sight.x.service.SightService/FinalizeEpisode',
                request_serializer=service_dot_service__pb2.FinalizeEpisodeRequest.SerializeToString,
                response_deserializer=service_dot_service__pb2.FinalizeEpisodeResponse.FromString,
                )


class SightServiceServicer(object):
    """This API manages Sight logs, their creation and access to them.
    """

    def Test(self, request, context):
        """A test request to validate that the service is up.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def PrintInsertionTime(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def Create(self, request, context):
        """Creates a new Sight log.
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def Launch(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def GetWeights(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def DecisionPoint(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def CopyDataToReplayServer(self, request, context):
        """rpc DecisionOutcome(DecisionOutcomeRequest)
        returns (DecisionOutcomeResponse) {}

        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def CurrentStatus(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def ProposeAction(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def FinalizeEpisode(self, request, context):
        """Missing associated documentation comment in .proto file."""
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_SightServiceServicer_to_server(servicer, server):
    rpc_method_handlers = {
            'Test': grpc.unary_unary_rpc_method_handler(
                    servicer.Test,
                    request_deserializer=service_dot_service__pb2.TestRequest.FromString,
                    response_serializer=service_dot_service__pb2.TestResponse.SerializeToString,
            ),
            'PrintInsertionTime': grpc.unary_unary_rpc_method_handler(
                    servicer.PrintInsertionTime,
                    request_deserializer=service_dot_service__pb2.TestRequest.FromString,
                    response_serializer=service_dot_service__pb2.TestResponse.SerializeToString,
            ),
            'Create': grpc.unary_unary_rpc_method_handler(
                    servicer.Create,
                    request_deserializer=service_dot_service__pb2.CreateRequest.FromString,
                    response_serializer=service_dot_service__pb2.CreateResponse.SerializeToString,
            ),
            'Launch': grpc.unary_unary_rpc_method_handler(
                    servicer.Launch,
                    request_deserializer=service_dot_service__pb2.LaunchRequest.FromString,
                    response_serializer=service_dot_service__pb2.LaunchResponse.SerializeToString,
            ),
            'GetWeights': grpc.unary_unary_rpc_method_handler(
                    servicer.GetWeights,
                    request_deserializer=service_dot_service__pb2.GetWeightsRequest.FromString,
                    response_serializer=service_dot_service__pb2.GetWeightsResponse.SerializeToString,
            ),
            'DecisionPoint': grpc.unary_unary_rpc_method_handler(
                    servicer.DecisionPoint,
                    request_deserializer=service_dot_service__pb2.DecisionPointRequest.FromString,
                    response_serializer=service_dot_service__pb2.DecisionPointResponse.SerializeToString,
            ),
            'CopyDataToReplayServer': grpc.unary_unary_rpc_method_handler(
                    servicer.CopyDataToReplayServer,
                    request_deserializer=service_dot_service__pb2.CopyDataToReplayServerRequest.FromString,
                    response_serializer=service_dot_service__pb2.CopyDataToReplayServerResponse.SerializeToString,
            ),
            'CurrentStatus': grpc.unary_unary_rpc_method_handler(
                    servicer.CurrentStatus,
                    request_deserializer=service_dot_service__pb2.CurrentStatusRequest.FromString,
                    response_serializer=service_dot_service__pb2.CurrentStatusResponse.SerializeToString,
            ),
            'ProposeAction': grpc.unary_unary_rpc_method_handler(
                    servicer.ProposeAction,
                    request_deserializer=service_dot_service__pb2.ProposeActionRequest.FromString,
                    response_serializer=service_dot_service__pb2.ProposeActionResponse.SerializeToString,
            ),
            'FinalizeEpisode': grpc.unary_unary_rpc_method_handler(
                    servicer.FinalizeEpisode,
                    request_deserializer=service_dot_service__pb2.FinalizeEpisodeRequest.FromString,
                    response_serializer=service_dot_service__pb2.FinalizeEpisodeResponse.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'sight.x.service.SightService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))


 # This class is part of an EXPERIMENTAL API.
class SightService(object):
    """This API manages Sight logs, their creation and access to them.
    """

    @staticmethod
    def Test(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/Test',
            service_dot_service__pb2.TestRequest.SerializeToString,
            service_dot_service__pb2.TestResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def PrintInsertionTime(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/PrintInsertionTime',
            service_dot_service__pb2.TestRequest.SerializeToString,
            service_dot_service__pb2.TestResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def Create(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/Create',
            service_dot_service__pb2.CreateRequest.SerializeToString,
            service_dot_service__pb2.CreateResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def Launch(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/Launch',
            service_dot_service__pb2.LaunchRequest.SerializeToString,
            service_dot_service__pb2.LaunchResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def GetWeights(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/GetWeights',
            service_dot_service__pb2.GetWeightsRequest.SerializeToString,
            service_dot_service__pb2.GetWeightsResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def DecisionPoint(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/DecisionPoint',
            service_dot_service__pb2.DecisionPointRequest.SerializeToString,
            service_dot_service__pb2.DecisionPointResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def CopyDataToReplayServer(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/CopyDataToReplayServer',
            service_dot_service__pb2.CopyDataToReplayServerRequest.SerializeToString,
            service_dot_service__pb2.CopyDataToReplayServerResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def CurrentStatus(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/CurrentStatus',
            service_dot_service__pb2.CurrentStatusRequest.SerializeToString,
            service_dot_service__pb2.CurrentStatusResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def ProposeAction(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/ProposeAction',
            service_dot_service__pb2.ProposeActionRequest.SerializeToString,
            service_dot_service__pb2.ProposeActionResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)

    @staticmethod
    def FinalizeEpisode(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        return grpc.experimental.unary_unary(request, target, '/sight.x.service.SightService/FinalizeEpisode',
            service_dot_service__pb2.FinalizeEpisodeRequest.SerializeToString,
            service_dot_service__pb2.FinalizeEpisodeResponse.FromString,
            options, channel_credentials,
            insecure, call_credentials, compression, wait_for_ready, timeout, metadata)
