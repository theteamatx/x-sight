...
class GeneticAlgorithm(OptimizerInstance):
    ...
    def find_best_worst(
            self, options: List[Dict[str,
                                     Any]]) -> Tuple[float, int, float, int]:
    ...
    def find_best_worst_probweighted(
        ...
        sum_of_max_adjusted_outcomes = largest_outcome * len(
            options) - sum_outcomes
        smallest_outcome_choice = random.uniform(0,
        ...
        if (len(self.ga_population) < self.max_population_size
            ...
            if self.proposals and random.randint(0, 10) < 5:
                ...
                if random.randint(0, 9) > 4:
                    ...
                    logging.info('%s|    at cross_idx=%d', request.worker_id,
                                 cross_idx)
                    for i, key in enumerate(keys):
                        if i < cross_idx:
                            next_action[key] = self.ga_population[spouse_idx][
                                'action'][key]
                        else:
                            next_action[key] = self.ga_population[largest_idx][
                                'action'][key]
                    algorithm = 'mating'
                else:
                ...
                logging.info('%s| new next_action=%s', request.worker_id,
                             next_action)
    ...
    def current_status(
        ...
        for i, unit in enumerate(
                sorted(self.ga_population,
                       key=lambda p: p['outcome'],
                       reverse=True)):
            response += (f'  {i},{unit["outcome"]:.5F},' +
                         ','.join([str(unit['action'][key])
                                   for key in keys]) + '\n')
