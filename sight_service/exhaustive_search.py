...
class ExhaustiveSearch(OptimizerInstance):
  ...
  """
    ...
    def decision_point(
        ...
        if self.last_sample:
            ...
            for i, key in reversed(list(enumerate(keys))):
                ...
                if self.next_sample_to_issue[i] < len(
                        self.possible_values[key]) - 1:
    ...
    def finalize_episode(
        ...
        logging.info('FinalizeEpisode complete_samples=%s' %
                     self.complete_samples)

        # if(self.max_reward_sample == {} or self.max_reward_sample['outcome'] < request.decision_outcome.outcome_value):
        if (self.max_reward_sample == {} or self.max_reward_sample['reward']
    ...
    def current_status(
        ...
        while not reached_last:
            ...
            if tuple(cur) in self.complete_samples:
                response += ', ' + str(
                    self.complete_samples[tuple(cur)]['outcome'])
            else:
                response += ', ?'
            response += '\n'
