  ...
  """
    ...
    def _attr_summary(
        ...
        if attr.min_value < attr.max_value:
            return (
                f'"{key}": {{ "description": {attr.description}, "min_value":'
    ...
    def launch(
        ...
        logging.info(
            'INTERACTIVE=%s',
            self._llm_config.goal == sight_pb2.DecisionConfigurationStart.
        ...
        if (self._llm_config.goal == sight_pb2.DecisionConfigurationStart.
            ...
            self._intro += (detail_prompt +
                            self._ask(self._intro + detail_prompt) + '\n')
    ...
    def _random_state(self) -> Dict[str, float]:
        ...
        for key, p in self.state.items():
            s[key] = (p.max_value -
                      p.min_value) * random.random() + p.min_value
        return s
    ...
    def _hist_event_to_text(self, event: Dict, last_outcome: float,
        ...
        if event['outcome'] is not None:
            ...
            t += 'Simulation Outcome (json format): ' + str(
                event['outcome']) + '\n'
            if (self._llm_config.goal != sight_pb2.DecisionConfigurationStart.
                    LLMConfig.LLMGoal.LM_INTERACTIVE):
    ...
    def _ask(self, prompt) -> str:
        ...
        elif (self._llm_config.algorithm == sight_pb2.
              DecisionConfigurationStart.LLMConfig.LLMAlgorithm.LA_CHAT_BISON):
            return self._ask_chat_bison(prompt)
        elif (self._llm_config.algorithm == sight_pb2.
              DecisionConfigurationStart.LLMConfig.LLMAlgorithm.LA_GEMINI_PRO):
    ...
    def _action_from_text_bison(self,
                                worker_id: str) -> List[Dict[str, float]]:
    ...
    def _action_from_chat_bison(self,
                                worker_id: str) -> List[Dict[str, float]]:
        while True:
            text = self._ask_chat_bison(self._intro,
                                        self._history_to_chat(worker_id))
            logging.info('text=[%s]', text)
            try:
                return [json.loads(text)]
            except json.decoder.JSONDecodeError:
    ...
    def _action_from_gemini_pro(self,
                                worker_id: str) -> List[Dict[str, float]]:
        while True:
            logging.info('ask_geminipro')
            prompt = self._intro + '\n'
            random_sample, _ = self._hist_event_to_text(
                self._random_event(), None, False)
    ...
    def decision_point(
        ...
        if (self._llm_config.goal == sight_pb2.DecisionConfigurationStart.
                LLMConfig.LLMGoal.LM_INTERACTIVE and self._waiting_on_tell):
            self._lock.release()
        ...
        self._history.append({
            'state':
            self._params_to_dict(request.decision_point.state_params),
            'action':
            None,
            'outcome':
            None,
        ...
        elif (self._llm_config.goal != sight_pb2.DecisionConfigurationStart.
              LLMConfig.LLMGoal.LM_INTERACTIVE and len(self._history) > 1
    ...
    def finalize_episode(
        ...
        if (self._llm_config.goal == sight_pb2.DecisionConfigurationStart.
            ...
            if len(self._actions_to_do) == 0:
                can_respond_to_question, response = self._is_done(
                    request.worker_id)
    ...
    @overrides
    def tell(self,
             request: service_pb2.TellRequest) -> service_pb2.TellResponse:
    ...
    def current_status(
        ...
        return service_pb2.CurrentStatusResponse(
            response_str=
            f"""[LLM: script={self._intro + self._history_to_text(None)}
-----------------
