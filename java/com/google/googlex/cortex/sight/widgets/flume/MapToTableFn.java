...
/** Wraps Flume MapToTableFn class with a Sight loggable variant. */
public abstract class MapToTableFn<T, K0, V0>
    extends FJ.MapToTableFn<Pair<SightId, T>, K0, Pair<SightId, V0>> {
  private static final GoogleLogger logger = GoogleLogger.forEnclosingClass();

  public abstract Pair<K0, V0> map(T input, Sight sight);

  @Override
  public Pair<K0, Pair<SightId, V0>> map(Pair<SightId, T> input) {
    try (DoFnProcessBlock block =
        DoFnProcessBlock.create(derivedClassName, input.getFirst(), sight)) {
      if (emitFn == null) {
        emitFn = EmitToTableFn.create(/* stageId= */ System.identityHashCode(this), sight);
      }
      Pair<K0, V0> output = map(input.getSecond(), sight);
      emitFn.emitEmitFn();
      Pair<K0, Pair<SightId, V0>> result =
          emitFn.getLoggedOutput(output.getFirst(), output.getSecond());
  ...
  protected MapToTableFn(Sight baseSight, String derivedClassName) {
  ...
  protected MapToTableFn(Params params) {
    this.params = params;
  }
