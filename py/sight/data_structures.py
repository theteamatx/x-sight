...
# from google3.googlex.cortex.sight.py.widgets.decision import decision
# from google3.googlex.cortex.sight.py.widgets.numpy_sight import numpy_sight
# from google3.googlex.cortex.sight.py.widgets.simulation import simulation_state
# from google3.googlex.cortex.sight.py.widgets.tensorflow_sight import tensorflow_sight
...
def log_var(name: str,
  ...
  # TODO: this is an unmaintainable mechanism for informing modules
...
def log(obj_to_log: Any,
  ...
  if isinstance(obj_to_log, str):
    ...
    sight.log_object(sight_obj, True)
  elif isinstance(obj_to_log, list) or isinstance(
      obj_to_log, tuple) or isinstance(obj_to_log, set):
    sight_obj.sub_type = sight_pb2.Object.SubType.ST_BLOCK_START
  ...
  elif isinstance(obj_to_log, np.int64) or isinstance(
      obj_to_log, np.float64) or isinstance(obj_to_log, bool):
...
def from_ordered_log(log_segment: List[sight_pb2.Object]) -> Any:
  ...
  if start.sub_type == sight_pb2.Object.SubType.ST_VALUE:
    ...
    if start.block_start.sub_type == sight_pb2.BlockStart.ST_LIST:
      if start.block_start.list.sub_type == sight_pb2.ListStart.ST_HETEROGENEOUS:
        list_objects = []
        i = 0
        while i < len(sub_log):
