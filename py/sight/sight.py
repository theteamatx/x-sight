...
class Sight(object):
  ...
  """
    ...
    def __init__(
        ...
        self.num_direct_contents = contextvars.ContextVar(
            'num_direct_contents')
        ...
        self.active_block_labels = contextvars.ContextVar(
            'active_block_labels')
        ...
        if self.params.in_memory:
            ...
            if self.params.local:
                self.path_prefix = '%s/%s' % (self.params.log_dir_path,
                                              path_label)
                self.id = 0
            ...
            if self.params.avro_output:
                ...
                try:
                    ...
                    if 'PARENT_LOG_ID' in os.environ:
                        ...
                        worker_location = os.environ[
                            'worker_location'].replace(':', '_')
                        self.path_prefix = (self.params.label + '_' +
                                            os.environ['PARENT_LOG_ID'] + '_' +
                                            'worker' + '_' + worker_location +
                                            '_' + 'log')
                        ...
                        self.path_prefix = (self.params.label + '_' + self.id +
                                            '_' + 'log' + '_run_mode')
                    else:
                ...
                self.avro_log_file_path = (
                    self.params.label + '_' + str(self.id) + '/' +
                    self.path_prefix
    ...
    def create_task(self, func):
        ...
        new_task = asyncio.create_task(go(func,
                                          state))  #, name=f'task_{task_id}')
        self.exit_block(
            f'asyncio.create_task: {asyncio.current_task().get_name()}',
            sight_pb2.Object(), frame)
        return new_task
        ...
        if self.avro_log:
            if self.avro_log.getbuffer().nbytes > 0:
                ...
                if self.avro_file_counter == 1:
                    create_external_bq_table(self.params, self.table_name,
                                             self.id)
                logging.info(
                    'Log GUI : https://script.google.com/a/google.com/macros/s/%s/exec?'
                    'log_id=%s.%s&log_owner=%s&project_id=%s',
                    self.SIGHT_API_KEY, self.params.dataset_name,
                    self.table_name, self.params.log_owner,
    ...
    """
        ...
        if self.is_binary_logged():
            return self.log_object(
                sight_pb2.Object(sub_type=sight_pb2.Object.SubType.ST_GAP),
                True)
    ...
    """
        ...
        if not self.active_block_labels.get() or self.location.get().size(
        ) == 1:
        ...
        if self.is_binary_logged():
            ...
            obj.block_end.num_direct_contents = self.num_direct_contents.get(
            ).pos()
    ...
    """
        ...
        if not values:
            logging.error('Failed to unset attribute %s, which is not set.',
                          key)
            return
