...
def build_push_service_img(sight_id):
    build_out = subprocess.run(
        [
            ...
            'build',
            '-t',
            f'gcr.io/{_PROJECT_ID}/{_SERVICE_PREFIX}' +
            sight_id,
            '-f',
            _DOCKER_FILE_PATH,
            '.',
...
def delete_service_img(sight_id):
  print('deleting image : gcr.io/' + _PROJECT_ID + '/' + _SERVICE_PREFIX + sight_id)
  subprocess.run(
            [
                'gcloud',
                'container',
...
def delete_service(service_name):
    ...
    cmd_args = ['gcloud', 'run', 'services', 'delete', sight_service_name, '--quiet']
    result = subprocess.run(args=cmd_args, capture_output=True, text=True)
    # print('result from deletion :', result)
    if result.returncode == 0:
        print(f'Successfully deleted Cloud Run service: {_SERVICE_PREFIX}{service_name}')
    else:
...
def run_experiment(sight_id, optimizer_value, image_id, table_queue):
    ...
    if (table_name and service_name):
        # print(result.stdout)
        table_queue.put((optimizer_value, table_name.group(1), service_name.group(1)))
    # else:
