  ...
  def process(self, task: Tuple[Any, Dict[str,
    ...
    for loc in sorted(time_steps):
      ...
      if last_decision_point_ts:
        logging.info(
            'choice_params=%s',
            last_decision_point_ts['decision_point'].decision_point.
            choice_params,
        )
...
def main(argv):
  ...
  if gfile.Glob(str(_OUT_FILE.value) + '.simulation_time_series*'):
    simulation_time_series = (root | beam.io.textio.ReadFromText(
    ...
    _ = (simulation_time_series | 'simulation_time_series' >>
         beam.io.WriteToText(str(_OUT_FILE.value) + '.simulation_time_series'))

  _ = (simulation_time_series | beam.Filter(lambda x: x[0] == 0) |
       beam.ParDo(TimeSeriesToTfExample()) | capacitorio.WriteToCapacitor(
           str(_OUT_FILE.value) + '.examples', ProtoCoder(sight_pb2.Object)))

  _ = (simulation_time_series | beam.Filter(lambda x: x[0] == 1) |
