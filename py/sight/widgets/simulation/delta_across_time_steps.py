...
class SimulationTimeSeries(beam.DoFn):
  ...
  def process(
      self, task: Tuple[Any, Dict[str, Any]]
    ...
    for state_named_val in task[1]['simulation_states_and_named_value_objects']:
      for ancestor_loc in reversed(
        ...
        if ancestor_loc in time_steps:
          time_steps[ancestor_loc]['objects'][state_named_val['object'][0]] = (
              state_named_val['object'][1]
          )
          break
    ...
    for dp in task[1]['decision_point']:
      ...
      if decision_point_vars is None:
        decision_point_vars = dp[
            'decision_point'
        ].decision_point.choice_params.keys()
    ...
    yield 1, {
        'inputs_params': pd.DataFrame(data=inputs_params, columns=param_vars),
        'inputs_state_vars': pd.DataFrame(
            data=inputs_state_vars, columns=state_vars
        ),
        'inputs_decision': pd.DataFrame(
            data=inputs_decision, columns=decision_point_vars
        ),
        'outputs_state_vars': pd.DataFrame(
            data=outputs_state_vars, columns=state_vars
        ),
        'outputs_outcome': pd.DataFrame(
            data=outputs_outcome, columns=['decision_outcome']
        ),
    }
...
class TimeSeriesToTfExample(beam.DoFn):
  ...
  def process(
      self,
      task: Tuple[int, Iterable[Tuple[Dict[str, float], Dict[str, float]]]],
  ) -> Iterator[sight_pb2.Object]:
    for row in task[1]:
...
def main(argv):
  ...
  if gfile.Glob(str(_OUT_FILE.value) + '.simulation_time_series*'):
    ...
    ) = analysis_utils.create_simulation_states_params_and_named_value_objects(
