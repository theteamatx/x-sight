...
_IN_CLUSTERS_FILE = gpath_flag.DEFINE_path(
    'in_clusters_file',
    None, ('Input file that contains the Sight log that documents how the '
...
class LogVarSequence(beam.DoFn):
  ...
  def process(
    ...
    for cluster_id in sorted(cluster_to_variables):
      ...
      for v in cluster_variables:
        logging.info('%s: v["values"](%s)=%s', v['parameters'],
                     v['values'].shape, v['values'])
...
class AggregateLogs(beam.DoFn):
  ...
  def process(
      self, task: Tuple[str, Iterable[Tuple[str,
                                            List[numpy_sight.LabeledNpArray]]]]
  ) -> None:
    """Time-orders the sequence of objects for a given simulation attribute.
...
def load_log_uid_clusters(
  ...
  if _IN_CLUSTERS_FILE.value:
    ...
    for clusters_fname in gfile.Glob(_IN_CLUSTERS_FILE.value):
      for message in read_capacitor_file(clusters_fname, [
          '*',
      ], 60):
...
def main(argv: Sequence[str]) -> None:
  ...
  simulation_log_uid: beam.pvalue.PCollection[
      analysis_utils.KeyedObjMap] = analysis_utils.create_log_uid_key(
          'simulation_and_cluster', 'simulation', simulation)

  clusters_key_log_uid = load_log_uid_clusters(root, simulation_log_uid)
  _ = (
      clusters_key_log_uid
      | 'clusters_key_log_uid' >>
      beam.io.WriteToText(str(_OUT_FILE.value) + '.clusters_key_log_uid'))
  ...
  simulation_and_cluster: beam.pvalue.PCollection[analysis_utils.ObjMap] = (
      ...
      | 'simulation_and_cluster CoGroupByKey' >> beam.CoGroupByKey()
      | beam.ParDo(
  ...
  _ = (
      ...
      | 'simulation_time_step' >>
      beam.io.WriteToText(str(_OUT_FILE.value) + '.simulation_time_step'))

  simulation_and_parameter_objects = analysis_utils.create_simulation_and_parameter_objects(
      log, objects_with_ancestors, simulation_and_cluster_sim_loc_uid,
      simulation_parameters, named_value, str(_OUT_FILE.value))
  _ = (
  ...
  simulation_time_step_state_objects = analysis_utils.create_simulation_time_step_state_objects(
      objects_with_ancestors, simulation_and_cluster_sim_loc_uid,
      simulation_time_step, simulation_state, named_value, str(_OUT_FILE.value))
  _ = (
