...
class LogVarSequence(beam.DoFn):
    ...
    """
        ...
        logging.info('LogVarSequence cluster_to_variables=%s',
                     cluster_to_variables)
        cluster_vars = []
        for cluster_id in sorted(cluster_to_variables):
            ...
            for v in cluster_variables:
                var_params.append(', '.join([
                    f'{p}={dict(v["parameters"])[p]}'
                    for p in ordered_varied_parameters
                ]))
...
class AggregateLogs(beam.DoFn):
    ...
    def process(
        self,
        task: Tuple[str, Iterable[Tuple[str,
                ...
                with Attribute('variable', var[0], sight):
                    ...
                    for i in range(len(var_clusters)):
                        with Attribute('cluster', str(i), sight):
                            data_structures.log_var(var[0], var_clusters[i],
                                                    sight)
...
def sight_encode_value(val: int) -> sight_pb2.Object:
  ...
  """
    with Sight(sight_pb2.Params(capacitor_output=True,
                                in_memory=True)) as sight:
    ...
    if _IN_CLUSTERS_FILE.value:
        ...
        for clusters_fname in gfile.Glob(_IN_CLUSTERS_FILE.value):
            for message in read_capacitor_file(
                ...
                cluster_assignment_log.ParseFromString(
        ...
        log_to_cluster_id = []
        for log_uid, cluster_id in cluster_assignment[
                'cluster_assignment'].items():
            log_to_cluster_id.append((log_uid, {
                'cluster_id':
                sight_encode_value(cluster_id)
            }))
        return root | beam.Create(log_to_cluster_id)
    else:
        return simulation_log_uid | beam.Map(
            lambda x: (x[0], {
...
def main(argv: Sequence[str]) -> None:
    ...
    simulation_and_cluster: beam.pvalue.PCollection[analysis_utils.ObjMap] = (
        ...
        | beam.ParDo(
            analysis_utils.CombineRecords('simulation',
    ...
    _ = (simulation_time_step_state_objects_in_time_order
         | 'simulation_time_step_state_objects_in_time_order' >>
         beam.io.WriteToText(
             str(_OUT_FILE.value) +
             '.simulation_time_step_state_objects_in_time_order'))
    ...
    _ = (simulation_params_and_vars
         | 'simulation_params_and_vars' >> beam.io.WriteToText(
             str(_OUT_FILE.value) + '.simulation_params_and_vars'))

    _ = (
        analysis_utils.create_var_key('simulation_params_and_vars',
                                      simulation_params_and_vars)
        |
