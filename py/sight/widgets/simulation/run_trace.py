...
def read_capacitor_file(
    filename: str, fields: Sequence[str] = ('*',), timeout: float = 60.0
) -> Iterator[Any]:
...
def load_log_uid_clusters(
  ...
  if _IN_CLUSTERS_FILE.value:
    ...
    if not cluster_assignment:
      logging.error(
          'Failed to find a clustering with %d clusters.', _NUM_CLUSTERS.value
      )
      return
    ...
    for log_uid, cluster_id in cluster_assignment['cluster_assignment'].items():
      log_to_cluster_id.append(
          (log_uid, {'cluster_id': sight_encode_value(cluster_id)})
      )
    return root | beam.Create(log_to_cluster_id)
  else:
    return simulation_log_uid | beam.Map(
        lambda x: (x[0], {'cluster_id': sight_encode_value(0)})
    )
...
def main(argv: Sequence[str]) -> None:
  ...
  _ = (
      ...
      | 'write simulation_and_cluster_sim_loc_uid'
      >> beam.io.WriteToText(
