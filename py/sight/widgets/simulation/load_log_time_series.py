...
def run_query(
    query_file_name: str, bq_client: bigquery.Client,
    params: Dict = dict()) -> pd.DataFrame:
  current_script_directory = os.path.dirname(os.path.abspath(__file__))
  with open(os.path.join(current_script_directory,
                         query_file_name + '.sql')) as file:
...
def load_table(table_name: str, bq_client: bigquery.Client) -> pd.DataFrame:
  ...
  """
  return bq_client.query(build_query(
      f'SELECT * FROM `{table_name}`',)).to_dataframe()
...
def main(argv: Sequence[str]) -> None:
  ...
  if os.path.isfile(_OUTFILE.value + '.states'):
    ...
    autoreg_variables = load_table(
        f'cameltrain.sight_logs.{_LOG_ID.value}_autoreg_all_vars_log',
        bq_client)['label'].tolist()
    boundary_variables = load_table(
        f'cameltrain.sight_logs.{_LOG_ID.value}_boundary_all_vars_log',
        bq_client)['label'].tolist()
    initial_variables = load_table(
        f'cameltrain.sight_logs.{_LOG_ID.value}_initial_all_vars_log',
        bq_client)['label'].tolist()
    print('autoreg_variables(#%d)=%s' %
          (len(autoreg_variables), autoreg_variables))
    print('boundary_variables(#%d)=%s' %
          (len(boundary_variables), boundary_variables))
    print('initial_variables(#%d)=%s' %
          (len(initial_variables), initial_variables))

    for type in ['autoreg', 'boundary', 'initial']:
      run_query('sim_unordered_time_series', bq_client, {'type': type})

    run_query(
        'sim_ordered_time_series', bq_client, {
            'num_autoreg_vars': len(autoreg_variables),
            'num_boundary_vars': len(boundary_variables),
            'num_initial_vars': len(initial_variables),
        })

    extract_job = bq_client.extract_table(
        bigquery.DatasetReference(_PROJECT_ID.value, 'sight_logs').table(
            f'{_LOG_ID.value}_simulation_ordered_time_series_log'),
    ...
    time_series = []
    for i, ts_file in enumerate(
        glob.glob(f'/tmp/*{_LOG_ID.value}.sim_ordered_time_series.*.csv')):
      print(ts_file)
      ...
      cur_ts[[f'autoreg:{v}' for v in autoreg_variables
             ]] = cur_ts['autoreg_values'].str.split(',', expand=True)
      cur_ts.drop(columns='autoreg_values', inplace=True)
      cur_ts[[f'boundary:{v}' for v in boundary_variables
             ]] = cur_ts['boundary_values'].str.split(',', expand=True)
      ...
      print('cur_ts.columns=%s=%s' %
            (len(cur_ts.columns), cur_ts.columns.tolist()))
