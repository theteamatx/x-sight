...
class SightAdder(base.Adder):
  ...
  """
    ...
    def reset(self, timeout_ms: Optional[int] = None):
        ...
        if (not self._existing_batch_last_record
    ...
    def observation_to_proto(self, observation: dict[str, Any]):
        ...
        if observation["next_timestep"].reward:
            obs.reward.CopyFrom(
                ndarray_to_proto(observation["next_timestep"].reward))
    ...
    def fetch_and_reset_observation_list(self, sight_client_id,
                                         sight_worker_id, learner_keys):
