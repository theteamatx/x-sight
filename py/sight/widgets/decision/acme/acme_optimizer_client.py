...
class AcmeOptimizerClient(OptimizerClient):
    ...
    def __init__(self, sight):
        super().__init__(
            sight_pb2.DecisionConfigurationStart.OptimizerType.OT_ACME)
        self._sight = sight
        self._actor = None
        self._adder = None
    ...
    @override
    def create_config(
            self) -> sight_pb2.DecisionConfigurationStart.ChoiceConfig:
    ...
    def generate_env_spec(
        ...
        if (attr_dict.valid_action_values):
            possible_values_list = list(
                attr_dict.valid_action_values.values())[0]
    ...
    """
        ...
        if (self._last_acme_action.dtype == 'int32'):
            self._last_acme_action = np.array(self._last_acme_action,
                                              dtype=np.int64)
