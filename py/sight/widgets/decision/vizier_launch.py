...
_BINARY_PATH = flags.DEFINE_string(
    'binary_path',
    None,
    ('Path of the Blaze target of the binary that contains the code to be configured.'
    ),
    required=True)

_REFERENCE_RUN_FILE = flags.DEFINE_string(
    'reference_run_file', None,
    ('File that contains the Sight log of a reference run.'))

_CELL = flags.DEFINE_string('cell', 'is',
                            'The Borg cell where the experiment will run')
