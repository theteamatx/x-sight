...
def get_worker_version(question_label):
  ...
  workers_config_path = os.getenv("WORKERS_CONFIG_PATH")#, "default/path/to/config.yaml")
  print('workers_config_path from env: ', workers_config_path)
  workers_config = load_yaml_config('/x-sight/'+workers_config_path)
  optimizers_config_path = os.getenv("OPTIMIZERS_CONFIG_PATH")#, "default/path/to/config.yaml")
