...
class AnalyzeSequence(beam.DoFn):
  ...
  def __init__(self, named_value_and_object_label: str,
               decision_point_label: str, decision_outcome_label: str,
  ...
  def process(
      self, task: Tuple[Any, Dict[str, Union[List[Any],
                                             List[Dict[str,
                                                       sight_pb2.Object]]]]]
  ) -> Iterator[Tuple[str, Tuple[List[Tuple[Dict[str, Any], float]], Dict[
      str, sight_pb2.DecisionConfigurationStart.StateProps], Dict[
          str, sight_pb2.DecisionConfigurationStart.StateProps],]]]:
    ...
    for cfg in task[1][self.configuration_label]:
      if cfg[
          'configuration'].block_start.configuration.sub_type == sight_pb2.ConfigurationStart.ST_DECISION_CONFIGURATION:
        if state_attrs:
          raise ValueError(
      ...
      if 'object' in obj:
        ...
        if last_decision_point:
          ...
          logging.info(
              ...
              observation, accumulated_outcome, last_decision_point)
...
class TrainOutcomePrediction(beam.DoFn):
  ...
  def process(
      self, task: Tuple[str, Iterable[Tuple[
    ...
    num_train_rows = int(num_total_rows * .8)
    ...
    with gfile.Open('/tmp/decision_outcomes.' + choice_label + '.csv',
      ...
      pd.DataFrame(
          np.concatenate(
              (input_array, np.reshape(output_array,
    ...
    for learner in [
      ...
      logging.info(
          '%s: mae=%s, rmse=%s', task[0], mae / abs(np.mean(eval_output_data)),
          math.sqrt(
    ...
    with io.BytesIO() as model_bytes:
      ...
      with Sight(
        ...
        scikit_learn_algorithm = sight_pb2.DecisionConfigurationStart.ScikitLearnAlgorithm(
        )
        scikit_learn_algorithm.model_encoding = model_bytes.getvalue()
        scikit_learn_algorithm.input_fields.extend(list(columns))

        choice_algorithm = sight_pb2.DecisionConfigurationStart.ChoiceAlgorithm(
        )
...
def main(argv):
  ...
  named_value_and_object = analysis_utils.create_log_uid_key(
      'named_values_to_objects log_uid_key', 'named_value',
  ...
  _ = (
      ...
      beam.io.WriteToText(str(_OUT_FILE.value) + '.named_value_and_object'))
  ...
  analyzed = (
      ...
      | 'named_value_and_object decision_point decision_outcome configuration '
      'CoGroupByKey' >> beam.CoGroupByKey()
      | 'named_value_and_object decision_point decision_outcome configuration '
      'AnalyzeSequence' >> beam.ParDo(
          AnalyzeSequence('named_value_and_object', 'decision_point',
                          'decision_outcome', 'configuration')))

  _ = (
      analyzed
      | 'analyzed' >> beam.io.WriteToText(str(_OUT_FILE.value) + '.analyzed'))
