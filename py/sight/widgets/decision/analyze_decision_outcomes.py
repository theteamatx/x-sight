    ...
    """
        ...
        for cfg in task[1][self.configuration_label]:
            if (cfg['configuration'].block_start.configuration.sub_type ==
                ...
                if state_attrs:
                    raise ValueError(
                        'Multiple decision configurations present in run %s' %
                        task[0])
        ...
        if state_attrs is None:
            raise ValueError('No decision configuration present in run %s' %
                             task[0])
        ...
        for obj in log:
            ...
            if 'object' in obj:
                ...
                logging.info('last_decision_point_state=%s',
                             last_decision_point_state)
                accumulated_outcome = 0
...
class TrainOutcomePrediction(beam.DoFn):
    ...
    def process(
        ...
        for dataset in task[1]:
            if state_attrs is None:
                ...
                if state_attrs != dataset[1] or action_attrs != dataset[2]:
                    raise ValueError(
                        'Inconsistent state/action attributes across runs.')
        ...
        train_idx, eval_idx = indices[:num_train_rows], indices[
            num_train_rows:]
        ...
        for learner in [
            ...
            logging.info('predicted_array%s=%s', predicted_array.shape,
                         predicted_array)
            mae = metrics.mean_absolute_error(eval_output_data,
                                              predicted_array)
            logging.info(
                '%s: mae=%s, rmse=%s',
                task[0],
                ...
                math.sqrt(
                    metrics.mean_squared_error(eval_output_data,
                                               predicted_array)) /
        ...
        with io.BytesIO() as model_bytes:
            ...
            with Sight(
                ...
                scikit_learn_algorithm = (sight_pb2.DecisionConfigurationStart.
                                          ScikitLearnAlgorithm())
                ...
                for attr_name, props in state_attrs.items():
                    decision_configuration.state_attrs[attr_name].CopyFrom(
                        props)
                for attr_name, props in action_attrs.items():
                    decision_configuration.action_attrs[attr_name].CopyFrom(
                        props)
